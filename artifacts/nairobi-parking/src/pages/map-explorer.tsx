...
import { SlidersHorizontal, Shield, Star, Zap, MapPin, Map, List, Heart } from "lucide-react";
        ...
        <div className="flex justify-between items-start gap-2 mb-1.5">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button
              ...
              aria-label={isFavorite(spot.id) ? "Remove from favorites" : "Save to favorites"}
            >
              <Heart
                className={`h-4 w-4 transition-colors ${isFavorite(spot.id) ? "fill-red-500 text-red-500" : "text-muted-foreground hover:text-red-400"}`}
              />
            </button>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="md:hidden flex border-b border-border bg-card flex-shrink-0">
        <button
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-medium transition-colors ${mobileView === "list" ? "text-primary border-b-2 border-primary" : "text-muted-foreground"}`}
          onClick={() => setMobileView("list")}
        >
          <List className="h-4 w-4" /> List
          {filteredSpots.length > 0 && <span className="text-xs bg-muted rounded-full px-1.5">{filteredSpots.length}</span>}
        </button>
        <button
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-medium transition-colors ${mobileView === "map" ? "text-primary border-b-2 border-primary" : "text-muted-foreground"}`}
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar — always visible on desktop, only in "list" mode on mobile */}
        <div className={`${mobileView === "list" ? "flex" : "hidden"} md:flex w-full md:w-[400px] border-r border-border bg-card flex-col h-full`}>
          {/* Search + filter bar */}
        ...
        {/* Map — full screen on mobile when toggled, always visible on desktop */}
        <div className={`${mobileView === "map" ? "flex" : "hidden"} md:flex flex-1 relative z-0`}>
          <MapView />
        </div>
      </div>
    </div>
